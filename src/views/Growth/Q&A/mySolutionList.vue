      ...
      <el-collapse-item :name="col.solutionId" :key="key" v-for="(col, key) in list">
        <template slot="title">
          ...
          <div class="header" @click.stop>
            <div class="title">
              <div>
                <div  :id="`solution${col.solutionId}`">{{col.title}}</div>
                <div class="operate">
                  ...
                  <el-button
                    type="text"
                    :disabled="activeName === col.solutionId"
                    @click="edit(col)"
                    v-if="col.askState === 3"
                  >编辑</el-button>
                </div>
...
export default class List extends Vue {
  ...
  edit(col: any) {
    this.activeName = col.solutionId;
  }
  ...
  async getQuestionDetail(solutionId: number) {
    this.isLoading = true;
  ...
  async answer() {
    const submitParams = {
      solutionId: this.activeName,
      content: this.quillValue,
      fileContent: this.answerDetail.fileContent,
    };
    const successMsg = '提交成功';
    const qAQuestionOpertRes = await Network.fetch<QAsolutioneditRes>(
      'qASolutionEdit',
      submitParams,
    );
  ...
  //   this.$nextTick(() => {
  //     if (this.queryId) {
  //       window.location.hash = 'solution' + this.queryId;
  //     }
  //   });
  // }
}
